...
// Use the host public context type; do not install a second agent/AI runtime.
type AgentMessage = ContextEvent["messages"][number];
type ToolResultMessage = Extract<AgentMessage, { role: "toolResult" }>;
type TextContent = Extract<ToolResultMessage["content"][number], { type: "text" }>;
...
export async function ensureStored(observation: Observation): Promise<void> {
	...
	try {
		...
		await handle.sync(); // OMP adaptation: complete the archive write before projecting a handle.
	} catch (error) {
...
export async function readRecallChunk(
	...
	if (!Number.isSafeInteger(offset) || offset < 0) throw new Error("Invalid recall byte offset");
	if (!Number.isSafeInteger(limits.maxBytes) || limits.maxBytes < 4 ||
		!Number.isSafeInteger(limits.maxLines) || limits.maxLines < 1) {
		throw new Error("Invalid recall page limits");
	}
