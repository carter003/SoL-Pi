...
import type { ProviderResult } from "../../../omp/reducer-provider.ts";
...
export function reducerInstructions(): string {
	return [
		"You are an evidence-preserving test/build output reducer.",
...
/**
 * Verify schema, source hash, observed status and exact quotes. Authentic
 * quotes do not prove completeness or the correctness of model-assigned kinds.
 */
...
export function receiptText(
	...
	lines.push(
		...
		"warning=Authentic quotes are not proof of completeness; omitted failures may exist",
		"readback=use OMP read on source_artifact with an explicit offset and limit when exact context is needed",
	);
	return lines.join("\n");
}
